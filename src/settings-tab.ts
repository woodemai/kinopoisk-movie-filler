...
export class KinopoiskMovieFillerSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl("h2", {
			text: "Настройки Kinopoisk Movie Filler",
		});

		new Setting(containerEl)
			.setName("Автозаполнение при открытии файла")
			.setDesc(
				"Автоматически заполнять информацию о фильме при открытии файла с ссылкой на Kinopoisk"
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Отображать постер как изображение")
			.setDesc(
				"Отображать постер как изображение в markdown вместо поля в frontmatter"
			)
			.addToggle((toggle) =>
				toggle
					.setValue(
						(this.plugin as any).settings.displayPosterAsImage
					)
					.onChange(async (value) => {
						(this.plugin as any).settings.displayPosterAsImage =
							value;
		...
		new Setting(containerEl)
			...
			.setDesc(
				"API ключ для доступа к Kinopoisk API (по умолчанию уже настроен)"
			)
			.addText((text) =>
				text
		...
		infoEl.innerHTML = `
			...
			<ul>
				...
				<li><code>Poster</code> - URL постера (если включено) или изображение постера в markdown</li>
			</ul>
		`;
	}
}
